...
# Functie om personen uit een bepaalde gemeente te tonen
def toon_personen_jonger_dan(leeftijd):
    gefilterde_personen = []

    # Gebruik een normale for-loop om de lijst te doorlopen
    for persoon in personen:
        if persoon[1] < int(leeftijd):
            gefilterde_personen.append(persoon)

    # Print de resultaten
    if gefilterde_personen:# als er minstens 1 item in lijst is.
        print(f"\nPersonen jonger dan {leeftijd}:")
        print(tabulate(gefilterde_personen, headers=kolommen, tablefmt="pretty_grid"))
    else:
        print(f"\nGeen personen gevonden janger dan {leeftijd} jaar.")


# Vraag de gebruiker om een leetijd in te voeren
leeftijd_input = input("Voer de maximum leeftijd in: ")
toon_personen_jonger_dan(leeftijd_input)
